...
local function GetMainStepCriteria()
    ...
    -- Get current stage for GetCriteriaInfoByStep fallback
    local currentStep = 1
    if C_Scenario.GetInfo then
        local okI, _, stage = pcall(C_Scenario.GetInfo)
        if okI and stage and stage > 0 then currentStep = stage end
    end
    local parts = {}
    local objectives = {}
    for criteriaIndex = 0, maxIdx do
        ...
        if (not cOk or not criteriaInfo) and C_ScenarioInfo and C_ScenarioInfo.GetCriteriaInfoByStep then
            cOk, criteriaInfo = pcall(C_ScenarioInfo.GetCriteriaInfoByStep, currentStep, criteriaIndex)
        end
...
local function ExecuteScenarioCriteriaUpdate()
    ...
    if not wasInScenario then return end  -- baseline not yet seeded; TryShowScenarioStart will re-request
...
local function TryShowScenarioStart()
    ...
    C_Timer.After(SCENARIO_DEBOUNCE, function()
        ...
        -- Catch criteria changes that fired during the debounce window
...
local function OnPlayerEnteringWorld()
    ...
    if not addon.Presence._scenarioInitDone then
        ...
        -- Seed criteria baseline so first SCENARIO_CRITERIA_UPDATE can diff properly
        if inScenario then
            local seedKey, seedObjs = GetMainStepCriteria()
...
local function OnScenarioCriteriaUpdate()
    ...
    TryShowScenarioStart()
    -- Always request; ExecuteScenarioCriteriaUpdate guards on IsScenarioActive and stateKey
    RequestScenarioCriteriaUpdate()
end
...
local function OnScenarioCompleted()
    ...
    if lastScenarioTitle and addon.GetDB and addon.GetDB("showScenarioEvents", true)
       and IsPresenceTypeEnabled("presenceScenarioComplete", "showScenarioEvents", true) then
        ...
        local L = addon.L or {}
        local subtitle
        -- Try to show the final objective's completion text (e.g. "Goldie Baronbottom Defeated")
        if lastScenarioObjectives and #lastScenarioObjectives > 0 then
            -- Walk backwards: the last objective with text is typically the final/boss objective
...
local function ExecuteRareDefeatedCheck()
    ...
    local now = GetTime()
    if (now - lastCombatTime) > RARE_COMBAT_WINDOW then return end
