...
-- and schedule a single refresh once ALL pending items have arrived.
...
local function OnItemLoaded(gen)
    ...
    if pendingItemLoads <= 0 and not refreshScheduled then
    ...
    if Item and Item.CreateFromItemID then
        ...
        if item then
            ...
            if item.ContinueOnItemLoad then
                pendingItemLoads = pendingItemLoads + 1
                item:ContinueOnItemLoad(function() OnItemLoaded(gen) end)
            end
        end
    end
...
local function DedupeAndAppend(raw, objectives, sectionHeader, sectionType)
    ...
    for _, agg in ipairs(DedupeByName(raw)) do
        local obj = {
            text         = agg.name,
...
-- Checks schematic fields first, then quality-tier item IDs as fallback.
-- @param recipeID number
-- @param isRecraft boolean|nil
-- @return number|nil Item quality (0-7)
    ...
    for _, item in ipairs(idList) do
        ...
        if type(recipeID) == "number" and recipeID > 0 then
            ...
            if showQualityInfo and supportsQualities and maxQuality and maxQuality > 0 then
                ...
                table.insert(objectives, 1, {
                    text = "Quality: " .. table.concat(pips, " ") .. " (1–" .. tostring(maxQuality) .. ")",
                    isQualityInfo = true,
                    finished = true, numFulfilled = 1, numRequired = 1,
                })
            end
